...
static uint8 XDATA I2C_MASTER_1_slReadBuf[I2C_MASTER_1_BTLDR_SIZEOF_READ_BUFFER];

/* I2C read buffer - The host reads responses from it */
static uint8 XDATA I2C_MASTER_1_slWriteBuf[I2C_MASTER_1_BTLDR_SIZEOF_WRITE_BUFFER];


/*******************************************************************************
* Function Name: I2C_MASTER_1_CyBtldrCommStart
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  I2C_MASTER_1_slWriteBuf - The global variable used to store a received
*                                command.
*  I2C_MASTER_1_slReadBuf -  The global variable used to store a response.
...
void I2C_MASTER_1_CyBtldrCommStart(void) CYSMALL 
{
    ...
    I2C_MASTER_1_SlaveInitWriteBuf(I2C_MASTER_1_slWriteBuf, I2C_MASTER_1_BTLDR_SIZEOF_WRITE_BUFFER);
    I2C_MASTER_1_SlaveInitReadBuf (I2C_MASTER_1_slReadBuf, 0u);
...
/*******************************************************************************
* Function Name: I2C_MASTER_1_CyBtldrCommStop
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: I2C_MASTER_1_CyBtldrCommReset
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: I2C_MASTER_1_CyBtldrCommWrite
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
cystatus I2C_MASTER_1_CyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut) CYSMALL
    ...
    if((NULL != pData) && (size > 0u))
    {
        ...
        *count = size;  /* Buffer was copied to I2C buffer */
        ...
        while(0u != timeoutMs)  /* Wait till response is read */
        {
            ...
            CyDelay(I2C_MASTER_1_WAIT_1_MS);
            timeoutMs--;
        }
    }
...
/*******************************************************************************
* Function Name: I2C_MASTER_1_CyBtldrCommRead
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  I2C_MASTER_1_slWriteBuf - The global variable used to store a
*                                received command.
*
*******************************************************************************/
         ...
         
{
    ...
    if((NULL != pData) && (size > 0u))
    {
        ...
        while(0u != timeoutMs)  /* Wait for command from host */
        {
            ...
            if(0u != (I2C_MASTER_1_slStatus & I2C_MASTER_1_SSTAT_WR_CMPLT))
            {
                /* Define how many bytes host has been written */
                byteCount = I2C_MASTER_1_slWrBufIndex;
                *count = (uint16) byteCount;

                /* Copy command to pData buffer */
                (void) memcpy((void *) pData, (void *) I2C_MASTER_1_slWriteBuf,
                              I2C_MASTER_1_MIN_UNT16(byteCount, size));
